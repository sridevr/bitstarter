...
var isURL = function(filename) {
    return filename.lastIndexOf("http", 0) == 0;
}

var assertFileExists = function(infile) {
    var instr = infile.toString();
    if(!isURL(instr) && !fs.existsSync(instr)) {
...
var cheerioHtmlFile = function(htmlfile, callback) {
  if (isURL(htmlfile)) {
    ...
    var html = cheerio.load(fs.readFileSync(htmlfile));
    callback(html);
  }
};
...
var checkHtmlFile = function(htmlfile, checksfile, callback) {
    cheerioHtmlFile(htmlfile, function(html) {
...
if(require.main == module) {
    program
        ...
        .option('-f, --file <html_file_or_URL>', 'Path to index.html', clone(assertFileExists), HTMLFILE_DEFAULT)
        .parse(process.argv);
